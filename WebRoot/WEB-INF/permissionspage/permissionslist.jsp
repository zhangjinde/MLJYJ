...
<style type="text/css">
  ...
  <button type="button" class="btn btn-success" id="addnew" style="margin-left: 25px;">添加权限</button>
    <script>
    $(function () {
        
		$('#addnew').click(function(){

				window.location.href="/SSH2_MLJYJ/permissions/permissionsAction_addUI.action";
		 });
  ...
  <div class="inline pull-right page">共<s:property value="pageBean.allRow"/> 条记录
        ...
        <s:else>
            <a href="permissionsActionList?page=1">第一页</a>
            <a href="permissionsActionList?page=<s:property value="%{pageBean.currentPage-1}"/>">上一页</a>
        </s:else>
        <s:if test="%{pageBean.currentPage != pageBean.totalPage}">
            <a href="permissionsActionList?page=<s:property value="%{pageBean.currentPage+1}"/>">下一页</a>
            <a href="permissionsActionList?page=<s:property value="pageBean.totalPage"/>">最后一页</a>
        </s:if>
        <s:else>
            下一页 最后一页
        </s:else></div>
  <s:action name="permissionsActionList" namespace="/permissions"></s:action><!-- executeResult="true"//处理结果包含到本页 -->
 ...
 <tr>
  ...
  <s:iterator value="pageBean.list">
   <tr>
            ...
            <td><s:property value="permission_describe"/></td>
            <s:url action="permissionsAction_del" namespace="/permissions" var="delurl">
             <s:param name="permissions_id" value="permissions_id"/>
            </s:url>
            <s:url action="permissionsAction_update" namespace="/permissions" var="updateurl">
              <s:param name="permissions_id" value="permissions_id"/>
            </s:url>
